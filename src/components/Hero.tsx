...
export const Hero = ({ listingCount = 0 }: HeroProps) => {
  const navigate = useNavigate();

  const badgeText =
    listingCount > 0
      ? `${listingCount} item${listingCount === 1 ? "" : "s"} listed — UK's pre-loved marketplace`
      : "Now open — UK's trusted pre-loved marketplace";
      ...
      <div className="container relative z-10">
        <div className="mx-auto max-w-4xl text-center">
          ...
          <p className="mb-12 text-xl md:text-2xl text-white/95 animate-fade-in drop-shadow-lg leading-relaxed" style={{ animationDelay: "0.2s" }}>
            Fashion, vintage, handmade and more — buy safely or start selling from home.
            <br className="hidden md:block" />
            UK dispatch address · secure Stripe payments · earn Velvet Coins.
          </p>
          ...
          <div className="mt-12 flex flex-wrap items-center justify-center gap-6 text-sm animate-fade-in" style={{ animationDelay: "0.4s" }}>
            ...
            <div className="flex items-center gap-2 text-white/90">
              <span className="text-2xl">✓</span>
              <span className="font-medium">UK Shipping (Evri, Royal Mail, InPost)</span>
            </div>
            <div className="flex items-center gap-2 text-white/90">
              <span className="text-2xl">✓</span>
              <span className="font-medium">Velvet Coin Rewards</span>
            </div>
          </div>
        </div>
      </div>
